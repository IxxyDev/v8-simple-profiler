...
function generateSummary(results) {
  ...
  const avgExecutionTime =
    validResults.reduce((sum, r) => sum + r.timing.mean, 0) / validResults.length;
  ...
  const optimizedCount = validResults.filter(
    r => r.optimization.available && r.optimization.flags?.optimized
  ).length;
...
function calculateOverallReliability(distribution, total) {
  const score =
    ((distribution.high || 0) * 3 + (distribution.medium || 0) * 2 + (distribution.low || 0) * 1) /
    total;
